...
public class TileMapHolder : MonoBehaviour
{
    ...
    public void SetGridPlaceStatus(GridPlace place, GridPlaceStatus status)
    {
        foreach (var cell in place.Place)
        {
            grid[cell.x, cell.y].IsOccupied = status == GridPlaceStatus.OCCUPIED;
        }
    }
    ...
    public bool IsBuildAvailable(Vector2Int gridPose, GroundPlacablePreview preview)
    {
            ...
            for (int x = 0; x < grid.GetLength(0); x++)
            {
                for (int y = 0; y < grid.GetLength(1); y++)
                {
                    if (grid[x, y] != null)
                    {
                        ...
                        Gizmos.DrawSphere(new Vector3(grid[x, y].centerX, grid[x, y].centerY, 0), 0.05f);
                    }
                }
            }
        }
